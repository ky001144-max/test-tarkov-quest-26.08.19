...
print("=== 1. Building PURE v1.10 (Tarkov.dev DATA ONLY) ===")
...
for q_id, q in tasks_dict.items():
    # Strict extraction from tasks_ko.txt / tasks.txt ONLY
    ...
    for idx, obj in enumerate(q.get("objectives", [])):
        ...
        # PURE extraction of zone positions from tasks.txt ONLY (NO hardcoding)
        zones = obj.get("zones", [])
        if zones:
            for z in zones:
                if z.get("position"):
...
pure_v10_quests.sort(key=lambda x: (x["required_level"], x["trader"]["id"], x["title_ko"]))

with open("app_v10/data/quests.json", "w", encoding="utf-8") as f:
    json.dump(pure_v10_quests, f, indent=2, ensure_ascii=False)

print(f"-> Successfully written PURE Tarkov.dev dataset to app_v10/data/quests.json ({len(pure_v10_quests)} quests)!")
